...
export class Categoriser {
         ...
         categorise(row: any, filter: string, category: Category, subcategory: string) {
           ...
           if (
             new RegExp(filter).test(row.Narration) ||
             new RegExp(filter).test(this.narrationMap[row.Narration])
           ) {
         ...
         categoriseAll(row: any) {
           ...
           this.categorise(
             row,
             '.*(REBEL|HBF RUN|SPORTS|GOOD LIFE|Hockey|Umpiring).*',
             Category.Health,
             'Sport',
           );
           ...
           this.categorise(
             row,
             '.*(BOOKS|AMAZON MKTPLC|BOOKDEPO|TREASA).*',
             Category.ClothingBooksGifts,
             'Books',
           );
           ...
           this.categorise(
             row,
             '.*(ENTERTAINMENT|Golden State Warriors).*',
             Category.FoodEntertainment,
             'Entertainment',
           );
           ...
           this.categorise(
             row,
             '.*(COMPLETE FIXED HL|COMPLETE VARIABLE HL).*',
             Category.Mortgage,
             'Mortgage Repayments',
           );
